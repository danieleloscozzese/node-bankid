...
const { Agent: CoreAgent } = require("node:https");
const { Agent: UAgent, fetch } = require("undici");
const request = require("request");
const BankID = require("./BankID.js");
const BankIDError = require("./BankIDError.cts");
...
class BankIDService {
  ...
  authenticate(args, callback) {
    if (args.personalNumber && !BankIDService.isValidSSN(args.personalNumber)) {
      return callback(
        new Error(
          "The specified social security number must be 12 digits in length."
        )
      );
    }
    ...
    request.post(
      ...
      (err, res) => {
        if (err || res.body.errorCode)
          return callback(BankIDService.createBankIDError(err, res.body));
